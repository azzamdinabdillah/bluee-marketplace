...
export default function HeaderBackoffice({ title, subtitle }: HeaderBackofficeProps) {
    return (
        <div className='flex flex-col md:flex-row gap-3 md:gap-5'>
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 md:p-5 lg:px-5 lg:py-[18px] bg-white rounded-2xl md:rounded-[18px] lg:rounded-[20px] w-full gap-4 md:gap-5 lg:gap-6">
                <div className="flex flex-col gap-1">
                    <h2 className="text-lg md:text-xl lg:text-2xl font-bold text-black-color font-lexend">{title}</h2>
                    <p className="text-sm md:text-[15px] lg:text-base font-semibold text-sec-color font-lexend">{subtitle}</p>
                </div>
